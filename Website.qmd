...
g_park_roads <- read_csv("~/Desktop/Thesis/g_park_roads.csv")
g_visitors <- read_csv("~/Desktop/Thesis/g_visitors.csv")
g_highway <- read_csv("~/Desktop/Thesis/g_highway.csv")
...
rutas <- st_read("~/Desktop/Thesis/rutas_all_roads.shp", quiet = TRUE)
highway <- st_read("~/Desktop/Thesis/highway.shp", quiet = TRUE)
tourist <- st_read("~/Desktop/Thesis/caminos2.shp", quiet = TRUE)
park_roads <-st_read("~/Desktop/Thesis/park_roads.shp", quiet = TRUE)
...
park_outline <- st_read("~/Desktop/Thesis/shapefiles_monteleon/pn_monteleon.shp", quiet = TRUE) %>%
  st_transform(st_crs(highway))

```
## Introduction

Animal movement is integral for the transportation of nutrients, plant productivity, and other
ecosystem functions. Anthropogenic disturbances such as urban developments and energy
production greatly affect animals’ daily movements, migration patterns, and access to necessary
resources. As human populations grow, it is important to analyze and identify how barriers, such
as roads, will affect animal movements. There is a great deal of variation on wildlife response to
roads and ungulates are among the more affected species. While there has been considerable
research about the impacts of roads on ungulates throughout North America and Europe, there
has been comparatively little research how roads impact South American wildlife.
...
- Guanacos will be more likely to cross the highway during the day because they are diurnal.
...
This will show if guanacos are not crossing the highway due to the concrete or lack of cover vs traffic, since concrete and lack of cover do not change during the night.
...
Highway: A two lane road that extends from Buenos Aires to the tip of Argentina, making it a major transportation route for the country with high volumes of traffic.

Visitors Road: A two lane road that takes tourists from the highway to the ocean through MLNP.

Park Use Roads: A dirt road system used only by park officials.
...
# shape <- st_read("~/Desktop/Thesis/shapefiles_monteleon/catastro_stacruz.shp", quiet = TRUE)
# shape_2 <- st_read("~/Desktop/Thesis/shapefiles_monteleon/pn_monteleon.shp", quiet = TRUE)
# 
...
#   st_as_sf(coords = c("easting", "northing"), crs = st_crs(highway))
# 
# guanacos <- read.csv('~/Desktop/Thesis/Thesis Presentation/raw_guanaco_data.csv')
# 
...
# 
# anim_save(filename = '~/Desktop/Website/www/guanaco_animate.mp4', animation = animation_g) 

```

## Model 
To analyze guanacos’ interaction with the roads I used Barrier Behavior Analysis (BaBA) a spatial analysis package that classifies barrier interaction types. 
...
```{r, message= F, warning=F, echo=F}
summary_highway <- read_csv("~/Desktop/Thesis/highway_interaction_table.csv") %>%
...
When these interactions are included, we find that guanacos are 7.4% and 7.7% less likely to cross the highway and visitor road, respectively, beyond the baseline likelihood of crossing during the day. 
...
This study builds on the idea that roads affect guanacos use of space negatively by
finding a mechanistic answer: guanacos fear traffic.
I can conclude that guanacos do not exhibit fear from being close to roads, but the road acts as
a barrier for crossing. 

This has huge conservation implications since roads may lead to spatial
segregation and isolation of guanaco populations. In North America and Europe, we have seen
the creation of wildlife tunnels and bridges for animals crossing major roads, and this could be a
potential mitigation technique in South America. Strategies such as these allow for increased
permeability of an otherwise uncrossable barrier.

The Patagonian steppe is considered a high priority location for global conservation due to
habitat fragmentation. There are currently lots of efforts to restore guanaco habitat.
Organizations such as Tompkins Conservation (also known as Rewilding Patagonia) have been
taking actions, such as buying and setting aside conservation tracts, to help protect habitat and
species, since much of the area has become farmland. Another strategy for guanaco conservation
is reintroducing guanacos throughout Argentina. When deciding locations of
conservation tracts or where to release guanacos it is important to consider their mobility. All
these conservation approaches will require huge investments so it is crucial to assess if roads in
the area will limit guanacos’ ability to move between parks or conservation tracts.